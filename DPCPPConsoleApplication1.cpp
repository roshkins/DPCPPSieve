...
IntVector VectorSieve(queue& q, long limit) {
	IntVector numbers;
	numbers.resize(limit);
	InitializeVector(numbers);
	range<1> num_items{ numbers.size() };
	buffer num_buffer(numbers);

	for (size_t i = 0; i < limit; i++) {
		q.submit([&](handler& h) {
			accessor num(num_buffer, h, read_write);
			h.parallel_for(num_items, [=](auto j) {
				auto candidate = num[j];
				if (candidate != 1 && candidate != num[i] && num[i] % candidate == 0) {
					num[i] = 0;
				}
			});
...
int main(int argc, char* argv[]) {
	...
	// Create alias of device selector for the device of your interest.
#if FPGA_EMULATOR
  ...
  // The default device selector will select the most performant device.
	...
	IntVector primes;
	primes.resize(vector_size);
	try {
		queue q(selector, exception_handler);
		primes = VectorSieve(q, vector_size);
		uint offset = 0;
		for (int i = 0; i < primes.size(); i++) {
			if (primes[i] > 0) {
				std::cout << "Prime #" << (++offset) << " is " << primes[i] << "\n";
			}
		}
	}
	catch (exception const& e) {
		std::cout << "An exception is caught for vector sieve.\n";
		std::terminate();
	}
